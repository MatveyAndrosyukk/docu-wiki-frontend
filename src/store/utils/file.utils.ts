    ...
    return nodes.map(
        node => {
            ...
            if (newNode.children) {
                ...
                newNode.children = closeAllFiles(
                    newNode.children
                );
            }
...
export function closeAllChildren(
    nodes: File[]
): File[] {
    ...
    return nodes.map(
        node => (
            {
                ...node,
                status: node.type === FileType.Folder
                    ? FileStatus.Closed
                    : node.status,
    ...
    return nodes.map(
        node => {
            ...
            if (newNode.type ===
                ...
                newNode.status = newNode.id === openedFileId
                    ? FileStatus.Opened :
                    FileStatus.Closed;
            }
            ...
            if (newNode.children &&
                ...
                newNode.children = closeAllFilesExcept(
                    newNode.children,
                    openedFileId
                );
            }
...
export const getAllChildFolderIdsUi = (
    node: UiFile
): number[] => {
...
export function findPathToFile(
    ...
    for (const file of files) {
        ...
        if (file.children) {
            ...
            const childPath = findPathToFile(
                file.children,
                targetId
            );
