    ...
    reducers: {
        toggleFolder(
            ...
            if (state.openedFolders.includes(id)) {
                ...
                if (node) {
                    const childFolderIds = getAllChildFolderIdsUi(node);
                    state.openedFolders = state.openedFolders.filter(
                        folderId => folderId !== id && !childFolderIds.includes(folderId)
                    );
                } else {
                    state.openedFolders = state.openedFolders.filter(folderId => folderId !== id);
                }
            } else {
                state.openedFolders.push(id);
            }
        },
        openFile(state, action: PayloadAction<number | null>) {
        ...
        openPathToNode(state, action: PayloadAction<{ id: number; files: any[] }>) {
            const targetId = action.payload.id;
            const files = action.payload.files;

            const path = findPathToFile(files, targetId);
            if (!path) return;

            const targetNode = findFileById(files, targetId);
            if (!targetNode) return;

            if (targetNode.type === FileType.File) {
                state.openedFileId = targetId;
                state.openedFolders = openFoldersOnPathPreserveOthers(state.openedFolders, path);
        ...
        markImageError(state, action: PayloadAction<string>) {
